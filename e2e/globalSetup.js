...
const createTestUser = () =>
  fetchAsAdmin('/users', {
    ...
    .then((resp) => {
      if (resp.status !== 200) {
        throw new Error(
          `Error: Could not authenticate test user - response ${resp.status}`
        );
      }
      return resp.json();
    })
...
const checkAdminCredentials = () =>
  fetch('/login', {
    ...
    .then((resp) => {
      if (resp.status !== 200) {
        throw new Error(
          `Error: Could not authenticate as admin user - response ${resp.status}`
        );
      }
    ...
    mongoGlobalSetup({ rootDir: __dirname })
      .then(async () => {
        ...
        process.on('uncaughtException', (err) => {
          ...
          console.error('Error al crear el user de prueba:', err);
